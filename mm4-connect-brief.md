# Build Brief — MM4 Connect: Full Experience
**Project:** HavenQuest
**Date:** June 2, 2026
**Status:** PENDING — Ready for Claude Code
**Priority:** High — core Navigator experience
...
MM4 — Connect currently shows a simple locked preview card. This brief builds out the full MM4 experience with four distinct sections:

1. The Welcome Moment — warm transition copy
2. Meet Your Market Director — placeholder introduction card
3. Your Family Profile — optional intake form with strong nudge
4. What Happens Next — strategy session preview

Supabase migration already executed — `family_profile` JSONB column is live.
...
MM4 currently renders from the shared locked preview component. MM4 needs its own dedicated component.

---

## New File to Create

`components/portal/milemarkers/MM4Connect.tsx`

---

## Update MileMarkerContent.tsx

Currently case 4 routes to the MM4to10 locked preview. When `currentMileMarker >= 4` (MM4 is active or complete), render `<MM4Connect />` instead.

```typescript
case 4:
  if (selectedMileMarker < currentMileMarker) {
    // completed summary
    return <CompletedCard name="Connect" description="Your Market Director was assigned and your relationship began." />
  }
  if (selectedMileMarker === currentMileMarker) {
    return <MM4Connect session={session} />
  }
  // locked preview — falls through to MM4to10
```

Pass `session` as a prop to MM4Connect so it can read and write to Supabase using the user's email.

---

## MM4Connect.tsx — Full Component
...
### FamilyProfile type (add to types/index.ts)
```typescript
export interface FamilyProfile {
  family_members: string[]        // ['partner', 'children', 'parents', 'pets']
  family_names: string            // optional free text — "Sarah, Jake (14), Emma (11)"
  children_ages: string[]         // ['under5', '5-10', '11-14', '15-18']
  special_needs: string           // free text, optional
  work_situation: string          // 'remote' | 'hybrid' | 'office' | 'self-employed' | 'retired' | 'other'
  commute_destination: string     // free text, optional
  school_preference: string[]     // ['public', 'private', 'homeschool', 'open']
  faith_community: string         // 'yes' | 'no' | 'open'
  activities: string              // free text — sports, arts, programs
  move_motivation: string         // 'job' | 'family' | 'lifestyle' | 'cost' | 'retirement' | 'other'
  biggest_concern: string         // free text, optional
  additional_notes: string        // free text, optional
}
```
...
Placeholder card — real content populated when Market Directors are recruited.
  ...
  <div className="flex items-start gap-4">
    ...
    <div className="flex-1">
      ...
      <p className="text-sm leading-relaxed" style={{ color: '#4B5563' }}>
        ...
        "tell me about yourself." It starts with real guidance from someone who
...
### Section 3 — Your Family Profile (intake form)

**The nudge (always visible above the form):**
```tsx
...
<div className="space-y-6">
  ...
  <div>
    <label className="block text-sm font-semibold mb-1" style={{ color: WARM_DARK }}>
      Would you like to share your family members' names? <span style={{ color: '#9A8E82', fontWeight: 400 }}>(optional)</span>
    </label>
    <p className="text-xs mb-2" style={{ color: '#9A8E82' }}>
      First names only — helps your Market Director personalize the conversation.
      Example: "Sarah, Jake (14), Emma (11)"
    </p>
    <input
      type="text"
  ...
  <div>
    <label className="block text-sm font-semibold mb-1" style={{ color: WARM_DARK }}>
      Any special needs, considerations, or specific programs you're looking for? <span style={{ color: '#9A8E82', fontWeight: 400 }}>(optional)</span>
    </label>
    <p className="text-xs mb-2" style={{ color: '#9A8E82' }}>
  ...
  <div>
    <label className="block text-sm font-semibold mb-1" style={{ color: WARM_DARK }}>
      What worries you most about this move? <span style={{ color: '#9A8E82', fontWeight: 400 }}>(optional)</span>
    </label>
    <p className="text-xs mb-2" style={{ color: '#9A8E82' }}>
  ...
  <div>
    <label className="block text-sm font-semibold mb-1" style={{ color: WARM_DARK }}>
      Anything else your Market Director should know? <span style={{ color: '#9A8E82', fontWeight: 400 }}>(optional)</span>
    </label>
    <textarea
...
## Color Constants (use existing from StarterPortal)
```typescript
...
- [ ] MM4Connect.tsx created as a new component
- [ ] FamilyProfile interface added to types/index.ts
- [ ] MileMarkerContent routes case 4 to MM4Connect when currentMileMarker >= 4
- [ ] Section 1 — Welcome moment renders with user's first name
- [ ] Section 2 — Market Director placeholder card renders
- [ ] Section 3 — Family profile form renders with all fields
- [ ] Family member toggle buttons work (multi-select)
- [ ] Children's ages section only shows when 'children' is selected in family members
- [ ] Optional name field renders
- [ ] Work situation single-select works
- [ ] School preference multi-select works
- [ ] Move motivation single-select works
- [ ] Free text areas work for special needs, biggest concern, additional notes
- [ ] Save button writes family_profile to Supabase
- [ ] On reload — family_profile loads from Supabase and form pre-populates
- [ ] Saved confirmation message appears after save
- [ ] Section 4 — What Happens Next renders
...
| File | Change |
|---|---|
| `types/index.ts` | Add FamilyProfile interface |
| `components/portal/milemarkers/MM4Connect.tsx` | CREATE — full new component |
| `components/portal/MileMarkerContent.tsx` | Route case 4 to MM4Connect when active |
